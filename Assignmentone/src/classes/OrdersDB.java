...
public class OrdersDB {
	...
	//TODO: Need to get all complete details
	public OrderItems getCompleteOrderItemDetails(int orderItemId) {
		...
		conn = db.getConn();
		try {
			String sql = "SELECT * FROM orderitems inner join performance on performance.Id = orderitems.PerformanceID"+
						 "inner join concert on concert.id = performance.concertID inner join"+
					     "TicketVenuePrices on TicketVenuePrices.performanceID = performance.Id" + 
						 "inner join venue on venue.Id = performance.venueID " +
						 "inner join `Ticket Types` on `Ticket Types`.`idTicket Types` = TicketVenuePrices.ticketTypeID" + 
					     "inner join orders on orders.id = orderitems.OrderId where Id = ?";
			ps = conn.prepareStatement(sql);
			...
			while(rs.next()) {
				...
				Venue ven = new Venue (rs.getInt("Id"), rs.getString("Name"), rs.getString("Address"), rs.getInt("ownerID"), rs.getString("City"), rs.getInt("totalSeats"), 
						               rs.getInt("clasOfSeats"), rs.getInt("typeOneSeats"), rs.getInt("typeTwoSeats"), rs.getString("State"), rs.getString("PostalCode"));
				...
				TicketVenuePrices tvp = new TicketVenuePrices(rs.getInt("idTicketVenuePrices"), rs.getInt("TicketPrice"), rs.getInt("venueID"), rs.getInt("ticketTypeID"), rs.getInt("performanceID"));
				TicketType ticketType = new TicketType(rs.getInt("idTicket Types"), rs.getString("SeatName"), rs.getString("Ticket Typescol"));
				Orders order = new Orders(rs.getInt("Id"), rs.getInt("CustomerId"), rs.getInt("TotalCost"), rs.getString("OrderDate"), rs.getString("BillingAddress"), rs.getString("CreditCardNumber"));
	...
	public void updateSeats(int seats, String venueName){
		DBAccessClass db = new DBAccessClass();
		db.connectMeIn();
		conn = db.getConn();
		try{
